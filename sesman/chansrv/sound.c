...
static HANDLE_AACENCODER s_fdk_aac_encoder = 0;
...
#include <opus/opus.h>
static OpusEncoder *s_opus_encoder = 0;
#endif

#if defined(XRDP_MP3LAME)
#include <lame/lame.h>
static lame_global_flags *s_lame_encoder = 0;
#endif

/* audio out: sound_server -> xrdp -> NeutrinoRDP */
static struct trans *s_audio_l_trans_out = 0; /* listener */
static struct trans *s_audio_c_trans_out = 0; /* connection */

/* audio in:  sound_server <- xrdp <- NeutrinoRDP */
static struct trans *s_audio_l_trans_in = 0;  /* listener */
static struct trans *s_audio_c_trans_in = 0;  /* connection */

static unsigned int s_training_sent_time = 0;
static int    s_cBlockNo = 0;
static int    s_bytes_in_stream = 0;
struct fifo  *g_in_fifo;
int    g_bytes_in_fifo = 0;
static int    s_time_diff = 0;
static int    s_best_time_diff = 0;


static struct stream *s_stream_inp = NULL;
static struct stream *s_stream_incoming_packet = NULL;

#define MAX_BBUF_SIZE (1024 * 16)
static char s_buffer[MAX_BBUF_SIZE];
static int s_buf_index = 0;
static unsigned int s_sent_time[256];

static int s_bbuf_size = 1024 * 8; /* may change later */

static struct list *s_ack_time_diff = 0;
...
static tui8 s_pcm_22050_data[] = { 0 };
static struct xr_wave_format_ex s_pcm_22050 =
{
    ...
    16,              /* bits per sample */
    0,               /* data size */
    s_pcm_22050_data /* data */
};

static tui8 s_pcm_44100_data[] = { 0 };
...
#if defined(XRDP_FDK_AAC)
static tui8 s_fdk_aac_44100_data[] = { 0 };
static struct xr_wave_format_ex s_fdk_aac_44100 =
{
    ...
    16,                  /* bits per sample */
    0,                   /* data size */
    s_fdk_aac_44100_data /* data */
};
#endif

#if defined(XRDP_OPUS)
static tui8 s_opus_44100_data[] = { 0 };
static struct xr_wave_format_ex s_opus_44100 =
{
    ...
    16,               /* bits per sample */
    0,                /* data size */
    s_opus_44100_data /* data */
};
#endif

#if defined(XRDP_MP3LAME)
static tui8 s_mp3lame_44100_data[] = { 0x01, 0x00, 0x02, 0x00, 0x00, 0x00, 0xb6, 0x00, 0x01, 0x00, 0x71, 0x05 };
static struct xr_wave_format_ex s_mp3lame_44100 =
{
    ...
    0,                      /* bits per sample */
    12,                     /* data size */
    s_mp3lame_44100_data    /* data */
};
#endif

static struct xr_wave_format_ex *s_wave_outp_formats[] =
{
    &s_pcm_44100,
    &s_pcm_22050,
#if defined(XRDP_FDK_AAC)
...
static int s_client_does_fdk_aac = 0;
static int s_client_fdk_aac_index = 0;

static int s_client_does_opus = 0;
static int s_client_opus_index = 0;

static int s_client_does_mp3lame = 0;
static int s_client_mp3lame_index = 0;

/* index into list from client */
static int s_current_client_format_index = 0;

/* index into list from server */
static int s_current_server_format_index = 0;

/* input formats */

static tui8 s_pcm_inp_22050_data[] = { 0 };
static struct xr_wave_format_ex s_pcm_inp_22050 =
{
    WAVE_FORMAT_PCM, /* wFormatTag */
    2,               /* num of channels */
    22050,           /* samples per sec */
    ...
    16,              /* bits per sample */
    0,               /* data size */
    s_pcm_inp_22050_data /* data */
};

static tui8 s_pcm_inp_44100_data[] = { 0 };
static struct xr_wave_format_ex s_pcm_inp_44100 =
{
    WAVE_FORMAT_PCM, /* wFormatTag */
    2,               /* num of channels */
    44100,           /* samples per sec */
    ...
    16,              /* bits per sample */
    0,               /* data size */
    s_pcm_inp_44100_data /* data */
};

static struct xr_wave_format_ex *s_wave_inp_formats[] =
{
    &s_pcm_inp_44100,
    &s_pcm_inp_22050,
    0
...
static int s_rdpsnd_can_rec = 0;

static int s_client_input_format_index = 0;
static int s_server_input_format_index = 0;
...
sound_send_server_output_formats(void)
{
    ...
    num_formats = sizeof(s_wave_outp_formats) /
                  sizeof(s_wave_outp_formats[0]) - 1;
    ...
    out_uint8(s, s_cBlockNo);               /* cLastBlockConfirmed */
    ...
    */
    ...
    {
        out_uint16_le(s, s_wave_outp_formats[index]->wFormatTag);
        out_uint16_le(s, s_wave_outp_formats[index]->nChannels);
        out_uint32_le(s, s_wave_outp_formats[index]->nSamplesPerSec);
        out_uint32_le(s, s_wave_outp_formats[index]->nAvgBytesPerSec);
        out_uint16_le(s, s_wave_outp_formats[index]->nBlockAlign);
        out_uint16_le(s, s_wave_outp_formats[index]->wBitsPerSample);
        bytes = s_wave_outp_formats[index]->cbSize;
        out_uint16_le(s, bytes);
        if (bytes > 0)
        {
            out_uint8p(s, s_wave_outp_formats[index]->data, bytes);
        }
    }
                            ...
                            int cbSize, char *data)
{
    ...
    if (wFormatTag == s_pcm_44100.wFormatTag &&
            nChannels == s_pcm_44100.nChannels &&
            nSamplesPerSec == s_pcm_44100.nSamplesPerSec &&
            nAvgBytesPerSec == s_pcm_44100.nAvgBytesPerSec &&
            nBlockAlign == s_pcm_44100.nBlockAlign &&
            wBitsPerSample == s_pcm_44100.wBitsPerSample)
    {
        s_current_client_format_index = aindex;
        s_current_server_format_index = 0;
    }
#if 0
    for (lindex = 0; lindex < NUM_BUILT_IN; lindex++)
    {
        ...
        {
            s_current_client_format_index = aindex;
            s_current_server_format_index = lindex;
        }
    }
#endif
    ...
    switch (wFormatTag)
    {
        case WAVE_FORMAT_AAC:
            ...
            s_client_does_fdk_aac = 1;
            s_client_fdk_aac_index = aindex;
            break;
        case WAVE_FORMAT_MPEGLAYER3:
            LOG_DEVEL(LOG_LEVEL_INFO, "wFormatTag, mp3");
            s_client_does_mp3lame = 1;
            s_client_mp3lame_index = aindex;
            break;
        case WAVE_FORMAT_OPUS:
            LOG_DEVEL(LOG_LEVEL_INFO, "wFormatTag, opus");
            s_client_does_opus = 1;
            s_client_opus_index = aindex;
            break;
    }
...
sound_wave_compress_fdk_aac(char *data, int data_bytes, int *format_index)
{
    ...
    if (s_client_does_fdk_aac == 0)
    {
        return rv;
    }

    if (s_fdk_aac_encoder == 0)
    {
        /* init fdk aac encoder */
        LOG_DEVEL(LOG_LEVEL_INFO, "sound_wave_compress_fdk_aac: using fdk aac");

        error = aacEncOpen(&s_fdk_aac_encoder, 0, 2);
        if (error != AACENC_OK)
        {
        ...
        error = aacEncoder_SetParam(s_fdk_aac_encoder, AACENC_AOT, aot);
        if (error != AACENC_OK)
        {
        ...
        sample_rate = s_fdk_aac_44100.nSamplesPerSec;
        error = aacEncoder_SetParam(s_fdk_aac_encoder, AACENC_SAMPLERATE,
                                    sample_rate);
        if (error != AACENC_OK)
        {
            LOG_DEVEL(LOG_LEVEL_INFO, "sound_wave_compress_fdk_aac: aacEncoder_SetParam() "
        ...
        mode = MODE_2;
        error = aacEncoder_SetParam(s_fdk_aac_encoder,
        ...
        error = aacEncoder_SetParam(s_fdk_aac_encoder, AACENC_CHANNELORDER,
                                    channel_order);
        if (error != AACENC_OK)
        {
            LOG_DEVEL(LOG_LEVEL_INFO, "sound_wave_compress_fdk_aac: aacEncoder_SetParam() "
        ...
        bitrate = s_fdk_aac_44100.nAvgBytesPerSec * 8;
        error = aacEncoder_SetParam(s_fdk_aac_encoder, AACENC_BITRATE,
                                    bitrate);
        if (error != AACENC_OK)
        {
            LOG_DEVEL(LOG_LEVEL_INFO, "sound_wave_compress_fdk_aac: aacEncoder_SetParam() "
        ...
        error = aacEncoder_SetParam(s_fdk_aac_encoder, AACENC_TRANSMUX, 0);
        if (error != AACENC_OK)
        {
            LOG_DEVEL(LOG_LEVEL_INFO, "sound_wave_compress_fdk_aac: aacEncoder_SetParam() "
        ...
        afterburner = 1;
        error = aacEncoder_SetParam(s_fdk_aac_encoder, AACENC_AFTERBURNER,
                                    afterburner);
        if (error != AACENC_OK)
        {
            LOG_DEVEL(LOG_LEVEL_INFO, "sound_wave_compress_fdk_aac: aacEncoder_SetParam() "
        ...
        error = aacEncEncode(s_fdk_aac_encoder, NULL, NULL, NULL, NULL);
        if (error != AACENC_OK)
        {
        ...
        error = aacEncInfo(s_fdk_aac_encoder, &info);
        if (error != AACENC_OK)
        {
    ...
    if (data_bytes < s_bbuf_size)
    {
        g_memset(data + data_bytes, 0, s_bbuf_size - data_bytes);
        data_bytes = s_bbuf_size;
    }
    ...
    error = aacEncEncode(s_fdk_aac_encoder, &in_buf, &out_buf,
                         &in_args, &out_args);
    if (error == AACENC_OK)
    {
        ...
        *format_index = s_client_fdk_aac_index;
...
sound_wave_compress_opus(char *data, int data_bytes, int *format_index)
{
    ...
    if (s_opus_encoder == 0)
    {
        /* NB (narrowband)       8 kHz
           ...
           FB (fullband)        48 kHz */
        s_opus_encoder = opus_encoder_create(48000, 2,
    ...
    cdata_bytes = opus_encode(s_opus_encoder, os16, data_bytes / 4,
                              cdata, cdata_bytes);
    if ((cdata_bytes > 0) && (cdata_bytes < data_bytes_org))
    {
        *format_index = s_client_opus_index;
        g_memcpy(data, cdata, cdata_bytes);
        rv = cdata_bytes;
    }
    g_free(cdata);
...
sound_wave_compress_mp3lame(char *data, int data_bytes, int *format_index)
{
    ...
    if (s_lame_encoder == 0)
    {
        ...
        s_lame_encoder = lame_init();
        if (s_lame_encoder == 0)
        {
            LOG_DEVEL(LOG_LEVEL_INFO, "sound_wave_compress_mp3lame: lame_init() failed");
            return rv;
        }
        lame_set_num_channels(s_lame_encoder, s_mp3lame_44100.nChannels);
        lame_set_in_samplerate(s_lame_encoder, s_mp3lame_44100.nSamplesPerSec);
        //lame_set_brate(s_lame_encoder, 64);
        lame_set_quality(s_lame_encoder, 7);
        if (lame_init_params(s_lame_encoder) == -1)
        {
        ...
        LOG_DEVEL(LOG_LEVEL_INFO, "                             brate            : %d", lame_get_brate(s_lame_encoder));
        LOG_DEVEL(LOG_LEVEL_INFO, "                             compression ratio: %f", lame_get_compression_ratio(s_lame_encoder));
        LOG_DEVEL(LOG_LEVEL_INFO, "                             encoder delay    : %d", lame_get_encoder_delay(s_lame_encoder));
        LOG_DEVEL(LOG_LEVEL_INFO, "                             frame size       : %d", lame_get_framesize(s_lame_encoder));
        LOG_DEVEL(LOG_LEVEL_INFO, "                             encoder padding  : %d", lame_get_encoder_padding(s_lame_encoder));
        LOG_DEVEL(LOG_LEVEL_INFO, "                             mode             : %d", lame_get_mode(s_lame_encoder));
    }
    ...
    cdata_bytes = lame_encode_buffer_interleaved(s_lame_encoder,
                  (short int *) data,
                  data_bytes / 4,
                  cdata,
                  cdata_bytes);
    ...
    {
        *format_index = s_client_mp3lame_index;
        g_memcpy(data, cdata, cdata_bytes);
        rv = cdata_bytes;
    }
...
sound_send_wave_data_chunk(char *data, int data_bytes)
{
    ...
    format_index = s_current_client_format_index;
    ...
    s_cBlockNo++;
    out_uint8(s, s_cBlockNo);
    s_sent_time[s_cBlockNo & 0xff] = time;

    LOG_DEVEL(LOG_LEVEL_DEBUG, "sound_send_wave_data_chunk: sending time %d, s_cBlockNo %d",
              time & 0xffff, s_cBlockNo & 0xff);
...
sound_send_wave_data(char *data, int data_bytes)
{
    ...
    if (s_time_diff > s_best_time_diff + 250)
    {
    ...
    while (data_bytes > 0)
    {
        space_left = s_bbuf_size - s_buf_index;
        ...
        g_memcpy(s_buffer + s_buf_index, data + data_index, chunk_bytes);
        s_buf_index += chunk_bytes;
        if (s_buf_index >= s_bbuf_size)
        {
            s_buf_index = 0;
            res = sound_send_wave_data_chunk(s_buffer, s_bbuf_size);
            if (res == 2)
            {
...
sound_process_training(struct stream *s, int size)
{
    ...
    time_diff = g_get_elapsed_ms() - s_training_sent_time;
...
sound_process_wave_confirm(struct stream *s, int size)
{
    ...
    time_diff = time - s_sent_time[cConfirmedBlockNo & 0xff];
    ...
    acc = 0;
    list_add_item(s_ack_time_diff, time_diff);
    if (s_ack_time_diff->count >= 50)
    {
        while (s_ack_time_diff->count > 50)
        {
            list_remove_item(s_ack_time_diff, 0);
        }
        for (index = 0; index < s_ack_time_diff->count; index++)
        {
            acc += list_get_item(s_ack_time_diff, index);
        }
        acc = acc / s_ack_time_diff->count;
        if ((s_best_time_diff < 1) || (s_best_time_diff > acc))
        {
            s_best_time_diff = acc;
        }
    }
    s_time_diff = acc;
    return 0;
...
process_pcm_message(int id, int size, struct stream *s)
{
    ...
    {
        case 0:
            if ((s_client_does_fdk_aac || s_client_does_mp3lame) && sending_silence)
            {
            ...
            break;
        case 1:
            if ((s_client_does_fdk_aac || s_client_does_mp3lame) && sending_silence == 0)
            {
                /* workaround for mstsc.exe. send silence data before send close */
                int send_silence_times = s_client_does_fdk_aac ? g_cfg->num_silent_frames_aac : g_cfg->num_silent_frames_mp3;  /* setting from sesman.ini */
                char *buf = (char *) g_malloc(s_bbuf_size, 0);
                if (buf != NULL)
                {
                    int i;
                    ...
                    {
                        g_memset(buf, 0, s_bbuf_size);
                        sound_send_wave_data_chunk(buf, s_bbuf_size);
                    }
                    free(buf);
                    s_time_diff = 0;
                }
...
sound_sndsrvr_sink_conn_in(struct trans *trans, struct trans *new_trans)
{
    ...
    if (trans != s_audio_l_trans_out)
    {
        return 1;
    }

    if (s_audio_c_trans_out != 0) /* if already set, error */
    {
        return 1;
    }
    ...
    s_audio_c_trans_out = new_trans;
    s_audio_c_trans_out->trans_data_in = sound_sndsrvr_sink_data_in;
    s_audio_c_trans_out->header_size = 8;
    trans_delete(s_audio_l_trans_out);
    s_audio_l_trans_out = 0;
...
sound_sndsrvr_source_conn_in(struct trans *trans, struct trans *new_trans)
{
    ...
    if (trans != s_audio_l_trans_in)
    {
        return 1;
    }

    if (s_audio_c_trans_in != 0) /* if already set, error */
    {
        return 1;
    }
    ...
    s_audio_c_trans_in = new_trans;
    s_audio_c_trans_in->trans_data_in = sound_sndsrvr_source_data_in;
    s_audio_c_trans_in->header_size = 8;
    trans_delete(s_audio_l_trans_in);
    s_audio_l_trans_in = 0;
...
sound_init(void)
{
    ...
    s_client_does_fdk_aac = 0;
    s_client_fdk_aac_index = 0;

    s_client_does_opus = 0;
    s_client_opus_index = 0;

    s_client_does_mp3lame = 0;
    s_client_mp3lame_index = 0;

    if (s_ack_time_diff == 0)
    {
        s_ack_time_diff = list_create();
    }
    list_clear(s_ack_time_diff);
...
sound_deinit(void)
{
    ...
    if (s_audio_l_trans_out != 0)
    {
        trans_delete(s_audio_l_trans_out);
        s_audio_l_trans_out = 0;
    }

    if (s_audio_c_trans_out != 0)
    {
        trans_delete(s_audio_c_trans_out);
        s_audio_c_trans_out = 0;
    }

    if (s_audio_l_trans_in != 0)
    {
        trans_delete(s_audio_l_trans_in);
        s_audio_l_trans_in = 0;
    }

    if (s_audio_c_trans_in != 0)
    {
        trans_delete(s_audio_c_trans_in);
        s_audio_c_trans_in = 0;
    }

#if defined(XRDP_MP3LAME)
    if (s_lame_encoder)
    {
        lame_close(s_lame_encoder);
        s_lame_encoder = 0;
        s_client_does_mp3lame = 0;
    }
#endif
              ...
              int total_length)
{
    ...
    if (!read_entire_packet(s, &s_stream_incoming_packet, chan_flags,
                            length, total_length))
    {
        return 0;
    }

    in_uint8(s_stream_incoming_packet, code);
    in_uint8s(s_stream_incoming_packet, 1);
    in_uint16_le(s_stream_incoming_packet, size);

    switch (code)
    {
        case SNDC_WAVECONFIRM:
            sound_process_wave_confirm(s_stream_incoming_packet, size);
            break;

        case SNDC_TRAINING:
            sound_process_training(s_stream_incoming_packet, size);
            break;

        case SNDC_FORMATS:
            sound_process_output_formats(s_stream_incoming_packet, size);
            break;

        case SNDC_REC_NEGOTIATE:
            sound_process_input_formats(s_stream_incoming_packet, size);
            break;

        case SNDC_REC_DATA:
            sound_process_input_data(s_stream_incoming_packet, size);
            ok_to_free = 0;
            break;
    ...
    if (ok_to_free && s_stream_incoming_packet)
    {
        xstream_free(s_stream_incoming_packet);
        s_stream_incoming_packet = NULL;
    }
...
sound_get_wait_objs(tbus *objs, int *count, int *timeout)
{
    ...
    if (s_audio_l_trans_out != 0)
    {
        objs[lcount] = s_audio_l_trans_out->sck;
        lcount++;
    }

    if (s_audio_c_trans_out != 0)
    {
        objs[lcount] = s_audio_c_trans_out->sck;
        lcount++;
    }

    if (s_audio_l_trans_in != 0)
    {
        objs[lcount] = s_audio_l_trans_in->sck;
        lcount++;
    }

    if (s_audio_c_trans_in != 0)
    {
        objs[lcount] = s_audio_c_trans_in->sck;
        lcount++;
    }
...
sound_check_wait_objs(void)
{
    if (s_audio_l_trans_out != 0)
    {
        if (trans_check_wait_objs(s_audio_l_trans_out) != 0)
        {
            LOG_DEVEL(LOG_LEVEL_DEBUG, "sound_check_wait_objs: s_audio_l_trans_out returned non-zero");
            trans_delete(s_audio_l_trans_out);
            s_audio_l_trans_out = 0;
        }
    }
    ...
    if (s_audio_c_trans_out != 0)
    {
        if (trans_check_wait_objs(s_audio_c_trans_out) != 0)
        {
            LOG_DEVEL(LOG_LEVEL_DEBUG, "sound_check_wait_objs: s_audio_c_trans_out returned non-zero");
            trans_delete(s_audio_c_trans_out);
            s_audio_c_trans_out = 0;
            sound_start_sink_listener();
        }
    ...
    if (s_audio_l_trans_in != 0)
    {
        if (trans_check_wait_objs(s_audio_l_trans_in) != 0)
        {
            LOG_DEVEL(LOG_LEVEL_DEBUG, "sound_check_wait_objs: s_audio_l_trans_in returned non-zero");
            trans_delete(s_audio_l_trans_in);
            s_audio_l_trans_in = 0;
        }
    }
    ...
    if (s_audio_c_trans_in != 0)
    {
        if (trans_check_wait_objs(s_audio_c_trans_in) != 0)
        {
            LOG_DEVEL(LOG_LEVEL_DEBUG, "sound_check_wait_objs: s_audio_c_trans_in returned non-zero");
            trans_delete(s_audio_c_trans_in);
            s_audio_c_trans_in = 0;
            sound_start_source_listener();
        }
...
sound_send_server_input_formats(void)
{
    ...
    num_formats = sizeof(s_wave_inp_formats) /
                  sizeof(s_wave_inp_formats[0]) - 1;
    ...
    */
    ...
    for (index = 0; index < num_formats; index++)
    {
        out_uint16_le(s, s_wave_inp_formats[index]->wFormatTag);
        out_uint16_le(s, s_wave_inp_formats[index]->nChannels);
        out_uint32_le(s, s_wave_inp_formats[index]->nSamplesPerSec);
        out_uint32_le(s, s_wave_inp_formats[index]->nAvgBytesPerSec);
        out_uint16_le(s, s_wave_inp_formats[index]->nBlockAlign);
        out_uint16_le(s, s_wave_inp_formats[index]->wBitsPerSample);
        bytes = s_wave_inp_formats[index]->cbSize;
        out_uint16_le(s, bytes);
        if (bytes > 0)
        {
            out_uint8p(s, s_wave_inp_formats[index]->data, bytes);
        }
    }
...
#if 1
    /* select CD quality audio */
    if (wFormatTag == s_pcm_inp_44100.wFormatTag &&
            nChannels == s_pcm_inp_44100.nChannels &&
            nSamplesPerSec == s_pcm_inp_44100.nSamplesPerSec &&
            nAvgBytesPerSec == s_pcm_inp_44100.nAvgBytesPerSec &&
            nBlockAlign == s_pcm_inp_44100.nBlockAlign &&
            wBitsPerSample == s_pcm_inp_44100.wBitsPerSample)
    {
        s_client_input_format_index = aindex;
        s_server_input_format_index = 0;
    }
#else
    /* select half of CD quality audio */
    if (wFormatTag == s_pcm_inp_22050.wFormatTag &&
            nChannels == s_pcm_inp_22050.nChannels &&
            nSamplesPerSec == s_pcm_inp_22050.nSamplesPerSec &&
            nAvgBytesPerSec == s_pcm_inp_22050.nAvgBytesPerSec &&
            nBlockAlign == s_pcm_inp_22050.nBlockAlign &&
            wBitsPerSample == s_pcm_inp_22050.wBitsPerSample)
    {
        s_client_input_format_index = aindex;
        s_server_input_format_index = 0;
    }
...
sound_input_start_recording(void)
{
    ...
    out_uint16_le(s, 2);
    out_uint16_le(s, s_client_input_format_index);
...
sound_sndsrvr_source_data_in(struct trans *trans)
{
    ...
    if (cmd == PA_CMD_SEND_DATA)
    {
        ...
        {
            if (s_stream_inp == NULL)
            {
                s_stream_inp = (struct stream *) fifo_remove_item(g_in_fifo);
                if (s_stream_inp != NULL)
                {
                    g_bytes_in_fifo -= s_stream_inp->size;
            ...
            {
                if (s_bytes_in_stream == 0)
                {
                    s_bytes_in_stream = s_stream_inp->size;
                }

                i = bytes_req - bytes_read;

                if (i < s_bytes_in_stream)
                {
                    //coverity[COPY_PASTE_ERROR:FALSE]
                    xstream_copyin(s, &s_stream_inp->data[s_stream_inp->size - s_bytes_in_stream], i);
                    bytes_read += i;
                    s_bytes_in_stream -= i;
                }
                else
                {
                    xstream_copyin(s, &s_stream_inp->data[s_stream_inp->size - s_bytes_in_stream], s_bytes_in_stream);
                    bytes_read += s_bytes_in_stream;
                    s_bytes_in_stream = 0;
                    xstream_free(s_stream_inp);
                    s_stream_inp = NULL;
                }
            }
        }
...
sound_start_source_listener(void)
{
    ...
    s_audio_l_trans_in = trans_create(TRANS_MODE_UNIX, 128 * 1024, 8192);
    s_audio_l_trans_in->is_term = g_is_term;
    g_snprintf(port, sizeof(port), CHANSRV_PORT_IN_STR, g_getuid(), g_display_num);
    s_audio_l_trans_in->trans_conn_in = sound_sndsrvr_source_conn_in;
    if (trans_listen(s_audio_l_trans_in, port) != 0)
    {
...
sound_start_sink_listener(void)
{
    ...
    s_audio_l_trans_out = trans_create(TRANS_MODE_UNIX, 128 * 1024, 8192);
    s_audio_l_trans_out->is_term = g_is_term;
    g_snprintf(port, sizeof(port), CHANSRV_PORT_OUT_STR, g_getuid(), g_display_num);
    s_audio_l_trans_out->trans_conn_in = sound_sndsrvr_sink_conn_in;
    if (trans_listen(s_audio_l_trans_out, port) != 0)
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "trans_listen failed");
    }
    return 0;
